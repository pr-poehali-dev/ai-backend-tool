...
import { ChatConfig } from '@/components/admin/useChatsState';
import { Assistant } from '@/components/admin/useAssistantsState';

interface CreateChatDialogProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onSubmit: (name: string, config: ChatConfig) => Promise<void>;
...
export const CreateChatDialog = ({ open, onOpenChange, onSubmit, assistants }: CreateChatDialogProps) => {
  const [name, setName] = useState('');
  const [config, setConfig] = useState<ChatConfig>(DEFAULT_CONFIG);
  const [isSubmitting, setIsSubmitting] = useState(false);

  const handleSubmit = async () => {
    if (!name.trim() || !config.assistantId) return;
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Создать чат-виджет</DialogTitle>
        </DialogHeader>
